...
### 2-3. 네이버 CLOVA OCR 키 발급 (한글 인식)
1. [Naver Cloud Platform](https://www.ncloud.com) 가입 → 콘솔.
2. **AI·NAVER API ▸ CLOVA OCR** → 도메인 생성(General/일반 OCR).
3. 생성된 도메인의 **APIGW Invoke URL**과 **Secret Key**를 위 스크립트 속성에 등록.
4. 키 미설정 시에도 **Tesseract.js**(브라우저 내장)로 자동 폴백됩니다. (한글 정확도는 낮지만 명단 매칭으로 보완)
